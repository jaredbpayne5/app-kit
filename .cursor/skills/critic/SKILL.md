...
1. Read `docs/PRD.md`, named exports in `docs/design-exports/`,
   `docs/design.md`, `AGENTS.md`, relevant current code, tests, and any
...
6. Surface material open questions. Recommend an answer when evidence
   supports one. Do not invent questions that cannot change the design.
7. Write `docs/critic.md`. Round 1 creates the file. Round 2 **appends**
   `# Critic — round 2` to the same file. Never overwrite round 1.
8. Stop after the verdict. Do not rewrite the proposal, plan the work,
...
## What I checked
PRD.md, design-exports/ (named files), design.md, relevant seams and routes.

## Findings

### 1. <Short title> (Blocker)
- Where: design.md §n, INV-n or AC-n
...
- A frame beats prose. If the design describes a new screen with no named
...
explaining, evaluating, or implementing the design. Omit other writing
preferences.
