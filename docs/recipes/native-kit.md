...
| `expo-localization` | Installed for locale-aware dates / currency when a screen needs it. `lib/dates.ts` uses `toLocaleDateString` (Intl) today. | plugin in `app.json`; wire a screen before relying on it |
...
| `expo-blur` | Installed for frosted overlays if a product screen needs them. Not used by the template paywall. | add `BlurView` when a screen needs it |
...
| --- | --- |
| `lib/streak.ts`, `lib/review-prompt.ts`, `lib/local-notifications.ts`, `lib/secure-storage.ts` | nothing imports them yet — tested seams, ready to wire |
